...
from utility.Tool import timer, ensureDir, SelfAttention
...
class PEG(AbstractRecommender):
    def __init__(self, sess, dataset, conf, train2cg=None, train2cg_idx=None):
        ...
        if self.dataset_name == 'LFM-0':
            self.context_feature_all = self.context_feature_mat[:, :]
            # self.item_feature_author = self.item_feature_mat[:, -1]
        else:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            # Text embedding in the same dimension
            # ==================================================================== #
            # text_feature = np.load('dataset/Huawei-blog/title.npy')
            # print(text_feature)
            # text_feature = tf.constant_initializer(text_feature)
            # self.item_text_embeddings = tf.get_variable("text_embeddings", shape=[self.num_items, 768],
            #                                             dtype=tf.float32, initializer=text_feature, trainable=False)
            # self.text_W = tf.Variable(initializer([768, self.hidden_factor]),
            #                           name='text_W', dtype=tf.float32)
            # self.item_text_embeddings = tf.matmul(self.item_text_embeddings, self.text_W)
            # ==================================================================== #
            if self.dataset_name == 'LFM-0':
                self.context_feature_embeddings = tf.Variable(initializer([self.num_context_features,
                                                                           self.hidden_factor]),
                                                              name='context_feature_embeddings', dtype=tf.float32)
                self.context_feature_bias = tf.Variable(initializer([self.num_context_features, 1]),
            ...
            # self.valid_text_embeddings = tf.gather(self.item_text_embeddings, list(range(self.num_valid_items)))
            if self.adj_norm_type in ['rs', 'rd', 'db']:
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            if self.item_feature_mat is not None:
                if self.dataset_name == 'Huawei-blog':
                    # TODO: here we need to try different setting: 1.only use user_emdeddings 2. use user_feature
                    item_feature_embedding_author = tf.nn.embedding_lookup(self.user_embeddings,
                                                                           self.item_feature_author)
                    all_item_feature_embedding = tf.concat([tf.expand_dims(self.valid_item_embeddings, 1),
                                                            tf.expand_dims(self.valid_text_embeddings, 1),
                                                            tf.expand_dims(item_feature_embedding_author, 1)],
                                                           axis=1)
                else:
            ...
            print('>>>>>>', self.context_feature_last.shape, np.array(self.context_feature_last).max())
            if self.dataset_name == 'LFM-0':
                self.context_feature_embedding = tf.nn.embedding_lookup(self.context_feature_embeddings,
                                                                        self.context_feature_all)  # [num_contexts, 4, h]
                self.context_feature_bias = tf.nn.embedding_lookup(self.context_feature_bias,
                                                                   self.context_feature_all)  # [num_contexts, 4, 1]
                self.encoded_context_embedding = tf.reduce_mean(self.context_feature_embedding,
                                                                axis=1)  # [num_contexts, h]
            else:
            ...
            # ==================================================================== #
            attention = []
            context_type = 4 if self.dataset_name == 'LFM-0' else 5
            for i in range(context_type):
                temp = tf.matmul(
                    tf.nn.relu(
                        tf.matmul(
            ...
            for k in range(1, self.num_gcn_layers + 1):
                ...
                #insts_kg_p_embedding = tf.gather(relation_intent, self.insts2userid)
                insts_kg_p_embedding = tf.nn.embedding_lookup(relation_intent, self.insts2userid)
                print("2222222222", insts_kg_p_embedding.shape)
                head, tail = self.edge_index[0].tolist(), self.edge_index[1].tolist()
                #edge_relation_emb = self.relation_embeddings[self.edge_type]
                ...
                if self.dataset_name == 'LFM-0':
                    insts_context_embedding_all = tf.nn.embedding_lookup(
                        layer_context_embedding, self.context_feature_all[self.insts2contextid])  # [num_insts, 4, h]
                    insts_event_embedding = tf.squeeze(
                        tf.matmul(tf.expand_dims(attention, 1), insts_context_embedding_all), 1)  # [num_insts, h]
                else:
                    insts_context_embedding_wo_last = tf.nn.embedding_lookup(
                        layer_context_embedding,
                        self.context_feature_wo_last[self.insts2contextid])  # [num_insts, 4, h]
                    ...
                    ####################### Attention Fusion 11 #################################
                    att_emb = tf.concat([tf.expand_dims(insts_event_embedding_1, 1),
                                         tf.expand_dims(insts_event_embedding_2, 1)], 1)  # [batch, 2]
                    Atten = SelfAttention(self.hidden_factor)
                    att_emb = Atten(att_emb)
                    print('>>>att_emb>>>>', att_emb.shape)
                    insts_event_embedding = tf.reduce_sum(att_emb, axis=1)
                    print('>>>layer_item_embedding>>>>', insts_event_embedding.shape)
                    #############################################################################
                ...
                if self.adj_norm_type in ['ls', 'db']:
                    ...
                    layer_entities_embedding = tf.math.unsorted_segment_mean(
                        ...
                        head,
                        self.num_entities,
                        name='aggregate_emtities_l%d' % k)
                    print("00000000000001111111111111 : mean ", layer_entities_embedding.shape)
                    layer_item_embedding_2 = layer_entities_embedding[:self.num_items]
                    # KG & UEG item embedding sum fusion
                    layer_item_embedding = (layer_item_embedding_1 * self.intent_weight + layer_item_embedding_2 * (1 - self.intent_weight))
                    # print('>>>layer_item_embedding_UEG>>>', layer_item_embedding_1.shape)
                    # print('>>>layer_item_embedding_KG>>>>', layer_item_embedding_2.shape)
                    # ####################### Attention Fusion 16 #################################
                    # att_emb = tf.concat([tf.expand_dims(layer_item_embedding_1, 1),
                    #                      tf.expand_dims(layer_item_embedding_2, 1)], 1)  # [batch, 2]
                    # Atten = SelfAttention(self.hidden_factor)
                    # att_emb = Atten(att_emb)
                    # print('>>>att_emb>>>>', att_emb.shape)
                    # layer_item_embedding = tf.reduce_sum(att_emb, axis=1)
                    # print('>>>layer_item_embedding>>>>', layer_item_embedding.shape)
                    # #############################################################################
                    if self.train2cg is not None:
                        ...
                        if self.dataset_name == 'LFM-0':
                            layer_context_embedding = tf.math.unsorted_segment_sqrt_n(
                                insts_context_embedding_new,
                                tf.reshape(self.context_feature_all[self.insts2contextid], [-1]),
            ...
            if self.dataset_name == 'LFM-0':
                self.context_feature_embedding = tf.nn.embedding_lookup(self.context_g_embeddings,
                                                                        self.context_feature_all)  # [num_contexts, 4, h]
            else:
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler(
        '%s/logs/%s-%s/%s_%s_att_11.log' % (args.proj_path, model_name, args.dataset, model_str, current_time))
    ...
    if args.grouping:
        ...
        Hyperparameters = ['model_name', 'dataset', 'epoch', 'batch_size', 'test_batch_size', 'test_interval',
                           ...
                           'merge_type', 'adj_norm_type', 'intent_weight']
