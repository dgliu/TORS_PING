...
from utility.Tool import timer, ensureDir, SelfAttention, FC
        ...
        with tf.name_scope("inference"):
            ...
            for k in range(1, self.num_gcn_layers + 1):
                ...
                if self.dataset_name == 'LFM-0':
                    ...
                    ########################## FC Fusion 11 ##########################
                    FC_layer = FC(self.hidden_factor)
                    fc_emb_2 = FC_layer(insts_event_embedding_2)
                    fusion_emb = insts_event_embedding_1 + fc_emb_2
                    print('>>>fusion_emb>>>>', fusion_emb.shape)
                    insts_event_embedding = fusion_emb
                ...
                if self.adj_norm_type in ['ls', 'db']:
                    ...
                    # ########################## FC Fusion 16 ##########################
                    # FC_layer = FC(self.hidden_factor)
                    # fc_emb_2 = FC_layer(layer_item_embedding_2)
                    # fusion_emb = layer_item_embedding_1 + fc_emb_2
                    # print('>>>fusion_emb>>>>', fusion_emb.shape)
                    # layer_item_embedding = fusion_emb
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler(
        '%s/logs/%s-%s/%s_%s_fc_11.log' % (args.proj_path, model_name, args.dataset, model_str, current_time))
