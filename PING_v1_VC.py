...
class PING(AbstractRecommender):
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.W_pinjie = tf.Variable(initializer([self.hidden_factor * 2, self.hidden_factor]),
                                                   name='W_pinjie',
                                                   dtype=tf.float32)  # (embedding_size, embedding_size)
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            for k in range(1, self.num_gcn_layers + 1):
                ...
                ##########################  Pinjie  #####################################
                insts_event_embedding = tf.concat([insts_event_embedding_1, insts_event_embedding_2], axis=-1)  # [,2d]
                insts_event_embedding = tf.matmul(insts_event_embedding, self.W_pinjie)   #[, d]
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler('%s/logs/%s-%s/%s_%s_11_pinjie.log' %
